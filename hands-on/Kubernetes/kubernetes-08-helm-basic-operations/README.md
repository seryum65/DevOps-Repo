...
```bash
helm create seryum-chart
```

- See the files of seryum-chart.
...
```bash
rm -rf seryum-chart/templates/*
```

- Create a `configmap.yaml` file under `seryum-chart/templates` folder with following content.

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: seryum-chart-config
data:
  myvalue: "Hello World"
```

- Install the seryum-chart.

```bash
helm install helm-demo seryum-chart
```
...
```bash
kubectl get cm
kubectl describe cm seryum-chart-config
```
...
- Let's create our own values and use it within the template. Update the `seryum-chart/values.yaml` as below.

```yaml
course: DevOps
```

- Edit the seryum-chart/templates/configmap.yaml as below.
...
metadata:
  name: seryum-chart-config
data:
  myvalue: "seryum-chart configmap example"
...
```bash
helm install --debug --dry-run mydryrun seryum-chart
```

- Install the seryum-chart.

```bash
helm install myvalue seryum-chart
```
...
```bash
helm install --debug --dry-run setflag seryum-chart --set course=AWS
```
...
```bash
helm install --debug --dry-run builtin-object seryum-chart
```

- Let's try more examples to get more clarity. Update the `seryum-chart/values.yaml` as below.
...
- Update the `seryum-chart/templates/configmap.yaml` as below.
...
```bash
helm install --debug --dry-run morevalues seryum-chart
```
...
- To add installation notes to your chart, simply create a `seryum-chart/NOTES.txt` file. This file is plain text, but it is processed like a template and has all the normal template functions and objects available.

- Let's create a simple `NOTES.txt` file under `seryum-chart/templates` folder.
...
```bash
helm install notes-demo seryum-chart
```
...
```bash
mkdir my-charts
mv seryum-chart-0.1.0.tgz my-charts
...
entries:
  ...
  - apiVersion: v2
    ...
    type: application
    urls:
    - http://<public ip>:8080/seryum-chart-0.1.0.tgz
    version: 0.1.0
...
```bash
cd my-charts
curl --data-binary "@seryum-chart-0.1.0.tgz" http://<public ip>:8080/api/charts
```
...
- Let's see how to maintain the chart version. In `seryum-chart/Chart.yaml`, set the `version` value to `0.1.1`and then package the chart.

```bash
helm package seryum-chart
mv seryum-chart-0.1.1.tgz my-charts
helm repo index my-charts --url http://<public-ip>:8080
```
...
```bash
cd my-charts
curl --data-binary "@seryum-chart-0.1.1.tgz" http://<public ip>:8080/api/charts
```
...
- In seryum-chart/Chart.yaml, set the `version` value to `0.2.0`and push the chart.

```bash
cd
helm cm-push seryum-chart mylocalrepo
```
...
```bash
helm cm-push seryum-chart mylocalrepo --version="1.2.3"
```
...
```bash
helm cm-push seryum-chart mylocalrepo --version="1.2.4"
```
...
helm repo update
helm search repo mylocalrepo
helm install from-local-repo mylocalrepo/seryum-chart
```
...
```bash
helm cm-push seryum-chart mylocalrepo --version="1.2.5"
helm repo update
```
...
```bash
helm upgrade from-local-repo mylocalrepo/seryum-chart
```
...
```bash
helm upgrade from-local-repo mylocalrepo/seryum-chart --version 1.2.4
```
